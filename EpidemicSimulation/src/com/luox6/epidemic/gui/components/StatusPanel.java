...
public class StatusPanel extends JPanel {
    ...
    private static final String SIMULATION_COUNT = "Current Tick(s): %d";
    ...
    public StatusPanel() {
        ...
        // Init
        updateSimulation(0, 0, 0, 0, 0);
        updateSimulationStatus(SIMULATION_STATUS.AWAIT_DATA);
    }

    /**
     * Update status bar current calculation number
     * @param times latest graph tick
     */
    public void updateSimulation(int times, int susceptible, int infected, int dead, int recovered) {
    ...
    public void updateSimulationStatus(SIMULATION_STATUS status) {
        switch (status) {
            ...
            case IN_PROGRESS -> statusLabel.setText(SIMULATION_IN_PROGRESS);
